...
if [[ -n "$SUB_TOKEN" ]]; then
  ...
  MIHOMO_SUB_LABELS+=("Mihomo 完整分流订阅" "Mihomo 纯节点订阅")
  MIHOMO_SUB_URLS+=(
    "https://${SUB_DIRECT_DOMAIN}/sub/${SUB_TOKEN}/mihomo-full.yaml"
    "https://${SUB_DIRECT_DOMAIN}/sub/${SUB_TOKEN}/mihomo-nodes.yaml"
  )
  MIHOMO_QR_FILES+=(
    "${USER_HOME}/subscription-mihomo-full.png"
    "${USER_HOME}/subscription-mihomo-nodes.png"
  )

  MIHOMO_LINK_LINES=""
  MIHOMO_QR_LINES=""
  for i in "${!MIHOMO_SUB_LABELS[@]}"; do
    MIHOMO_LINK_LINES+="${MIHOMO_SUB_LABELS[$i]}:\n${MIHOMO_SUB_URLS[$i]}\n\n"
    MIHOMO_QR_LINES+="${MIHOMO_SUB_LABELS[$i]}: ${MIHOMO_QR_FILES[$i]}\n"
  done
  printf -v MIHOMO_LINK_LINES '%b' "$MIHOMO_LINK_LINES"
  printf -v MIHOMO_QR_LINES '%b' "$MIHOMO_QR_LINES"
...
SUBLINKEOF
  ...
  for i in "${!MIHOMO_SUB_LABELS[@]}"; do
    echo "${MIHOMO_SUB_LABELS[$i]}: ${MIHOMO_SUB_URLS[$i]}"
  done
  ...
  if command -v qrencode >/dev/null 2>&1; then
    ...
    for i in "${!MIHOMO_SUB_LABELS[@]}"; do
      save_subscription_qr_png "${MIHOMO_SUB_URLS[$i]}" "${MIHOMO_QR_FILES[$i]}" && \
        info "${MIHOMO_SUB_LABELS[$i]} 二维码 PNG 已保存到 ${MIHOMO_QR_FILES[$i]}"
    done
    echo ""
    print_subscription_qr "V2RayN / Shadowrocket" "$V2RAYN_SUB_URL"
    for i in "${!MIHOMO_SUB_LABELS[@]}"; do
      print_subscription_qr "${MIHOMO_SUB_LABELS[$i]}" "${MIHOMO_SUB_URLS[$i]}"
    done
  else
...
build_cache_bypass_rule() {
  local domains=()
  local domain existing expr
  ...
  add_unique_domain "$DEFAULT_CDN_DOMAIN"
  add_unique_domain "$CDN_A"
  add_unique_domain "$CDN_B"
  ...
  for domain in "${domains[@]}"; do
    ...
    expr+="(http.host eq \"${domain}\")"
  done

  printf '%s or (http.request.uri.path contains "%s")' "$expr" "$XHTTP_PATH"
}

CACHE_BYPASS_RULE=$(build_cache_bypass_rule)
